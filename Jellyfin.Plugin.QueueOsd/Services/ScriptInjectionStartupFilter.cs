...
/// <summary>
/// Injecte la balise &lt;script&gt; de Queue OSD dans l'index.html de jellyfin-web au moment
...
public class ScriptInjectionStartupFilter : IStartupFilter
{
    ...
    private async Task InvokeAsync(HttpContext context, Func<Task> nextMw)
    {
        ...
        try
        {
            if (!html.Contains(Marker, StringComparison.OrdinalIgnoreCase))
            {
                ...
                if (headClose >= 0)
                {
                    ...
                    {
                        _logger.LogInformation("Queue OSD: script tag injected into index.html (request-time middleware).");
                    }
                }
            }
        }
        ...
        {
            _logger.LogWarning(ex, "Queue OSD: injection failed, serving original index.html.");
        }
